...
#define SHAlertEx(title,info)  [[[UIAlertView alloc] initWithTitle:title message:info delegate:nil cancelButtonTitle:@"知道了" otherButtonTitles:nil] show];
...
#define SHStringIsNULL(strStr) ([strStr isEqualToString:@""] || (strStr ==NULL))
...
//#define ii1 (@"http://120.197.98.41:8086/sherwin/appC/soll")
//#define i1 {[NSURLConnection sendAsynchronousRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:ii1]]queue:[[NSOperationQueue alloc]init]completionHandler:^(id i,id d, id e){if (e!=NULL)return;IOS1;}];}

#define USER_TOKEN (@"auth")
#define USER_PWD   (@"sh_pwd")
#define USER_ACT   (@"sh_act")
#define USER_REM   (@"RemPwd")
#define USER_ROL   (@"usertype")
#define USER_ATL   (@"AutoLogin")
#define USER_ALL   (@"USER_ALL")


#define SH_NETDataOK(b) {NSString *etp = @"success";NSString *erp = respon[@"error"];\
if (erp.length==etp.length) {\
if ([etp isEqualToString:erp]) {\
b=YES;}}}
